...
public class CFScheduler extends Scheduler {
	private static final int SCHED_WAIT_SCALE = 128;
	private static final long DEFAULT_TIMESLICE = 10;

	private LoadBalamce load;
	private List<Queue<Pcb>> queue;
	
	private class LoadBalamce extends JFrame {
	...
	public Pcb get(int cpuId) {
		...
		try {
			...
			long timeslice = waitingTime * SCHED_WAIT_SCALE / Pcb.getProcessCount();
			if (timeslice == 0) timeslice = DEFAULT_TIMESLICE;

			pcb.setTimeslice(timeslice);
			SwingUtilities.invokeLater(new Runnable() {
				@Override
				public void run() { load.repaint(); }
			});
			return pcb;
